      ...
      <div className="bg-white rounded-lg shadow-sm p-6 animate-fade-in">
        ...
        {selectedRole && selectedDepartment && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-xl font-semibold">
                Liste des {selectedRole === "encadrant" ? "encadrants" : "étudiants"} du département{" "}
            ...
            <Button variant="outline" onClick={generateCodes} className="mb-4">
              Générer les codes PFE
            </Button>
            <Button variant="outline" onClick={archiveList} className="mb-4">
              Archiver la liste
            </Button>
            ...
            <Table>
              ...
              <TableBody>
                {(selectedRole === "encadrant" ? filteredSupervisors : filteredStudents).map((person) => (
                  <TableRow key={person.id}>
                    <TableCell>{person.nom}</TableCell>
                    <TableCell>{person.prenom}</TableCell>
                    <TableCell>{person.codePFE || "Non généré"}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
